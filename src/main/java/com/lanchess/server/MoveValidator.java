...
public final class MoveValidator {
    ...
    public static void updateGameStatus(GameState state) {
        ...
        // Threefold repetition & 50-move rule HANYA relevan kalau game masih
        // berjalan (checkmate/stalemate sudah final, tidak perlu dicek lagi).
        // Kedua ini men-OVERRIDE status jadi DRAW kalau terpenuhi.
        if (state.getStatus() == GameStatus.PLAYING || state.getStatus() == GameStatus.CHECK) {
            int occurrences = state.recordCurrentPositionAndGetCount();
            if (occurrences >= 3) {
                state.setStatus(GameStatus.DRAW);
                state.setDrawReason(DrawReason.THREEFOLD_REPETITION);
            } else if (state.computeHalfmoveClock() >= 100) { // 50 langkah penuh = 100 half-move
                state.setStatus(GameStatus.DRAW);
                state.setDrawReason(DrawReason.FIFTY_MOVE_RULE);
            }
        }
    }
