...
public class StockfishEngine {
    ...
    /**
     * Minta evaluasi numerik posisi (centipawn, dari sudut pandang sisi yang
     * sedang jalan di FEN tsb) TANPA benar-benar mengeksekusi bestmove-nya -
     * dipakai bot untuk memutuskan terima/tolak tawaran seri (draw offer).
    ...
    public int evaluateCentipawns(String fen, int moveTimeMs) throws IOException {
        ...
        while ((line = readLine()) != null) {
            if (line.startsWith("info") && line.contains(" score ")) {
                Integer parsed = parseScore(line);
                if (parsed != null) lastScore = parsed;
            }
            if (line.startsWith("bestmove")) {
                break;
            }
        }
        return lastScore != null ? lastScore : 0;
    }

    private Integer parseScore(String infoLine) {
        String[] tokens = infoLine.trim().split("\\s+");
